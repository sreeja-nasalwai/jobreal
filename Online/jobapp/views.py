...
def single_job_view(request, job_id):  # Adjust function name if different
    ...
    if request.method == "POST" and request.POST.get("action") == "predict":
        ...
        # Pass dictionary to predict_job
        prediction = predict_job(job_data)
        return JsonResponse({"prediction": prediction})
    job.is_fraudulent = prediction
    job.save()
    
    # ... job_data preparation ...
    
    
    context = {"job": job}
    return render(request, "jobapp/job-single.html", context)  # Adjust template path if different
